...
int getDistance(string graph1_r, string graph2_r) {
string graph1 = "db/"+graph1_r;
string graph2 = "db/"+graph2_r;
// cout<<graph1_r<<" "<<graph2<<endl;
long long ans;
#ifndef NDEBUG
	...
	string database_option = graph1;
	string query_option = graph2;
	string mode_option = "astar"; //#kunttxueyawangjiel c++yufale suoyoude meiyou string yong # meifenhao 
	...
	if(1) {
		...
		for(ui i = 0;i < queries.size();i ++) {
			...
			if(current != pre) {
				// fprintf(stderr, "\r[%d%% fifinied]", current);
				fflush(stderr);
